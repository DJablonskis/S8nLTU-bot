const createSidePanel = () => {
  ...
  btnPower.onclick = (e) => {
    ...
    localStorage.setItem(BOT_POWER, BOT_ON ? OFF : ON);
    navigateTo(1);
  };
  ...
  if ("Notification" in window) {
    ...
    btnNotif.onclick = (e) => {
      if (Notification.permission === "denied") {
        return alert(
          "Notification permission was previously denied. Please enable notifications manualy if you want to use it."
        );
      } else if (!ON_N) {
        if (
          ...
          Notification.requestPermission().then((permission) => {
            if (permission === "granted") {
              localStorage.setItem(BOT_NOTIFICATIONS, ON_N ? OFF : ON);
              navigateTo(1);
            } else {
              return alert(
  ...
  btnStats.onclick = (e) => {
    localStorage.setItem(BOT_STATS, ON_S ? OFF : ON);
    navigateTo(1);
  };
